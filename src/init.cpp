...
#include "config/mypartycoin-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("mypartycoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "mypartycoin.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "mypartycoind.pid"));
#endif
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, mypartycoin, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all MyPartyCoin specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-backupzciv=<n>", strprintf(_("Enable automatic wallet backups triggered after each zMPC minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizemypartycoinamount=<n>     " + strprintf(_("Keep N MPC anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The MyPartyCoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("mypartycoin-loadblk");
...
/** Sanity checks
 *  Ensure that MyPartyCoin is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. MyPartyCoin Core is shutting down."));
...
#endif
    // Make sure only a single MyPartyCoin process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. MyPartyCoin Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("MyPartyCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // MyPartyCoin: load previous sessions sporks if we have them.
                ...
                // MyPartyCoin: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of MyPartyCoin Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart MyPartyCoin Core to complete") << "\n";
        ...
        bool fEnableZMPCBackups = GetBoolArg("-backupzciv", true);
        pwalletMain->setZMPCAutoBackups(fEnableZMPCBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    }
//
//    nAnonymizeMyPartyCoinAmount = GetArg("-anonymizemypartycoinamount", 0);
//    if (nAnonymizeMyPartyCoinAmount > 999999) nAnonymizeMyPartyCoinAmount = 999999;
//    if (nAnonymizeMyPartyCoinAmount < 2) nAnonymizeMyPartyCoinAmount = 2;
    ...
    LogPrintf("Anonymize MyPartyCoin Amount %d\n", nAnonymizeMyPartyCoinAmount);
