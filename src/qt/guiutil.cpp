...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a MyPartyCoin address (e.g. %1)").arg("MPCiHgGJJ6xGHqivmdZrYjRnhaYf6AJ2Mp"));
#endif
...
{
    // return if URI is not valid or is no MyPartyCoin: URI
...
{
    // Convert mypartycoin:// to mypartycoin:
    //
    //    Cannot handle this later, because mypartycoin:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open mypartycoin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "MyPartyCoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for MyPartyCoin.lnk
...
{
    return GetAutostartDir() / "mypartycoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a mypartycoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=MyPartyCoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the mypartycoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add mypartycoin app to startup item list
