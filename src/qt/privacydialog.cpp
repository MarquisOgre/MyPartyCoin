...
void PrivacyDialog::on_pushButtonZMPCControl_clicked()
{
    ZMPCControlDialog* zMPCControl = new ZMPCControlDialog(this);
    zMPCControl->setModel(walletModel);
    zMPCControl->exec();
}

void PrivacyDialog::setZMPCControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzMPCSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzMPC()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid MyPartyCoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    if (!ZMPCControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZMPCControlDialog::GetSelectedMints();
    }
    ...
    ZMPCControlDialog::listSelectedMints.clear();
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " MPC, ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->labelzMPCAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
