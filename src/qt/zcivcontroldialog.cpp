...
std::list<std::string> ZMPCControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZMPCControlDialog::listMints;

ZMPCControlDialog::ZMPCControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZMPCControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZMPCControlDialog::~ZMPCControlDialog()
{
    delete ui;
}

void ZMPCControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZMPCControlDialog::updateList()
{
...
void ZMPCControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZMPCControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZMPC_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZMPCControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZMPCControlDialog::GetSelectedMints()
{
...
void ZMPCControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
