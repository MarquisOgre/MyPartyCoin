...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total mypartycoin balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in mypartycoin/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in mypartycoin/kb\n"
    ...
    obj.emplace_back(Pair("zMPCsupply", zcivObj));
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"mypartycoinaddress\"\n"
            "\nReturn information about the given mypartycoin address.\n"
            "\nArguments:\n"
            "1. \"mypartycoinaddress\"     (string, required) The mypartycoin address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"mypartycoinaddress\", (string) The mypartycoin address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: MyPartyCoin address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are mypartycoin addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) mypartycoin address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"mypartycoinaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"mypartycoinaddress\"  (string, required) The mypartycoin address to use for the signature.\n"
